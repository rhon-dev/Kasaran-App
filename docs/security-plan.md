...
This is a first-class requirement (problem-brief describes real couples; REQ-SE-5 governs removal). The precise behaviour of **"revoke my partner's access and keep my data"**:

1. Removal of a partner is a **creator-initiated** action. Per REQ-SE-5, plan deletion is creator-only; partner removal normally needs two-party confirmation — **except** removal must not require the person being removed to consent, or a hostile ex could refuse forever. **Position: one-sided removal by the creator is permitted, is logged, and takes effect on the server immediately.** Two-party confirmation applies to *ownership transfer*, not to defensive removal. This resolves an ambiguity in REQ-SE-5 clause 4 and is flagged in §11 for confirmation.
...
| SEC-07 | One-sided defensive removal | The plan creator can remove a partner without the removed party's consent; action is logged (SEC-22). Verify: creator removes test partner B; B's `plan_members` row is gone and B's next sync is refused. | BACKEND |
...
| SEC-10 | Re-pairing after reinstall | A reinstalled app re-authenticates with the existing account and re-pulls from `since_hlc = 0`; no residual credential is needed from the old install. Verify: uninstall, reinstall, sign in, confirm full plan restored. | CLIENT |
...
| SEC-21 | Queued-write contents | A write that was queued offline carries its original HLC and the acting user's identity, and uploads under the current valid session token — not a token cached from when the write was made. Pass: make an offline write, rotate the session, reconnect → write uploads under the fresh token and is attributed correctly (REQ-OF-5, REQ-SE-4). Verify by inspecting the uploaded row's actor and the auth header. | BACKEND |
...
1. **★ One-sided removal (SEC-07) vs REQ-SE-5 clause 4.** This plan permits the creator to remove a partner without consent, overriding the two-party rule for defensive removal specifically, because a hostile ex would otherwise block it forever. Requirements say partner removal needs two-party confirmation. **These conflict.** Recommend amending REQ-SE-5 to scope two-party confirmation to ownership transfer only, and to allow one-sided creator removal. Confirm.
2. **★ Force-wipe on revocation (SEC-08).** Whether the removed partner's local copy is force-wiped, wipe-offered, or left. This plan offers wipe and does not force it, on the grounds that the removed person may have a legitimate interest in records of their own wedding and that force-wipe is unenforceable offline anyway. This intersects the SEC-33 erasure model — confirm the two are consistent.
3. **★ Erasure model (SEC-33, SEC-38).** The de-identify-B's-shared-copy vs full-delete split is a genuine DPA question with two data subjects over one record. Flagged for counsel, not resolved here.
4. **NPC registration threshold (SEC-37).** Applicability at student-project / early scale is uncertain. Verify; do not assume exemption.
5. **Non-creating partner's removal rights.** SEC-07 gives removal to the creator. Whether the non-creating partner can also defensively remove the creator (mutual right) is unspecified upstream. A wedding is symmetric; a creator-only removal right is not. Recommend a mutual defensive-removal right, but this needs a product decision and a REQ-SE-5 update.
